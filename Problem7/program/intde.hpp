//DE-Quadrature
//	from https://www.kurims.kyoto-u.ac.jp/~ooura/intde.html
//	Numerical Automatic Integrator for Improper Integral method : Double Exponential (DE) Transformation
//		dimension : one
//		table     : use
//functions
...
//		tiny      : minimum value that 1/tiny does not overflow (double)
...
//		relative error eps is relative error requested excluding cancellation of significant digits.
...
//		1. f(x) or (d/dx)^n f(x) has discontinuous points or sharp peaks over (a,b).
//			you must divide the interval (a,b) at this points.
//		2. relative error of f(x) is greater than eps.
//		3. f(x) has oscillatory factor and frequency of the oscillation is very high.
//
//intdei
//    [description]
...
public:
	...
	IntDE(int lenaw=8000,double tiny=1.0e-307):tiny(tiny)
	{
		aw.resize(lenaw);
	};
	~IntDE(){aw.clear();};
	void Initialize(double eps)
	{//IntDE.Integrate()で利用する変数
		double efs=0.1,hoff=8.5;
		int noff, nk, k, j;
		double pi2, xw, wg;
		double tinyln, epsln, h0, ehp, ehm, h, t, ep, em;
		pi2 = 2. * atan(1.0);
		tinyln = -std::log(tiny);
		...
		ehm = 1. / ehp;
		aw[2] = eps;
		aw[3] = std::exp(-ehm * epsln);
		aw[4] = std::sqrt(efs * eps);
		noff = 5;
		aw[noff] = 0.5;
		aw[noff + 1] = h0;
		...
		} while (2 * k - noff - 3 <= aw.size());
		aw[0] = k - 3;
	};
	double Integrate(std::function<double(double,void *)> f, double a, double b,double &err)
	{
		double value;
		...
		while (errd > errh && klim <= lenawm) {
			...
			errd = h * (fabs(value - 2 * iback) + fabs(ir - 2 * irback));
			h *= 0.5;
			m *= 2;
			klim = 2 * klim - noff;
		}
...
public:
	void *Parameter=nullptr;
	IntDEi(int lenaw=8000,double tiny=1.0e-307):tiny(tiny)
	{
		aw.resize(lenaw);
	}
	void Initialize(double eps)
	{
		...
		} while (2 * k - noff - 6 <= aw.size());
		aw[0] = k - 6;
	}
	double Integrate(std::function<double(double,void *)> f, double a, double &err)
	{
		double value;
		int noff, lenawm, nk, k, j, jtmp, jm, m, klim;
		...
		do {
			j += 6;
			fm = (f)(a + aw[j],Parameter);
			fp = (f)(a + aw[j + 1],Parameter);
